  ...
  it("update escrow", async () => {
    ...
    const instruction = await program.methods
      .updateOrder({
        ...
        makingAmount: null,
        takingAmount: null,
        expiredAt: new BN(Date.now() + 120), // extend  sec
      })
      .accounts({
        payer: payer.publicKey,
        maker: payer.publicKey,
    ...
    const size = calculateTransactionSize(tx1);
    console.log("Transaction size:", size);

    const sig = await rpc.sendTransaction(tx1);
    console.log("Order updated with transaction signature:", sig);

    await new Promise((resolve) => setTimeout(resolve, 2000));

    const txResponse = await program.provider.connection.getTransaction(sig, {
    ...
    if (txResponse && txResponse.meta) {
      const accountKeys = txResponse.transaction.message.getAccountKeys({
        accountKeysFromLookups: txResponse.meta.loadedAddresses,
      });

      const payerIndex = accountKeys.staticAccountKeys.findIndex(
        (k) => k.toBase58() === payer.publicKey.toBase58()
      );

      if (payerIndex === -1) {
        throw new Error("Payer not found in account keys");
      }

      const preLamports = txResponse.meta.preBalances[payerIndex];
      const postLamports = txResponse.meta.postBalances[payerIndex];

      const lamportsSpent = preLamports - postLamports;
      const solSpent = lamportsSpent / anchor.web3.LAMPORTS_PER_SOL;

      console.log("SOL spent by payer:", solSpent);
      console.log(
        "  - Additional lamports spent:",
        (lamportsSpent - txResponse.meta.fee) / anchor.web3.LAMPORTS_PER_SOL
      );
    }
  });
});
