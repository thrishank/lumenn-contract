...
describe("elara/partial_fill_order", () => {
  ...
  it("partial fill order", async () => {
    ...
    console.log("Partial Filling order ...");
    ...
    assert(
      makerBalanceAfter - makerBalanceBefore >= takingAmount.toNumber() / 2,
      "Tokens not correctly credited from maker"
    );

    assert(
      vaultBalanceBefore - vaultBalanceAfter >= takingAmount.toNumber() / 2,
    ...
    assertEscrowState({
      ...
      inputMint: input_mint,
      outputMint: output_mint,
      makingAmount: makingAmount.sub(new BN(inAmount)),
      takingAmount: takingAmount.div(new BN(2)),
    });
  });
});
